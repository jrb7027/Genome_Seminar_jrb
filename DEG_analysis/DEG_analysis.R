...
#alternate headers read
# Load necessary libraries
library(dplyr)
library(stringr)

# Read in the transcript header file
ttn <- read_delim("TTC_headers.txt", delim = " ", col_names = FALSE)

# Extract target_id, gene, and product using regular expressions
ttn <- ttn %>%
  mutate(
    target_id = str_extract(X1, "^[^|]+"),  # Extracts everything before the first "|"
    gene = str_extract(X1, "(?<=\\|)[^|]+"),  # Extracts text between the first and second "|"
    product = str_extract(X1, "(?<=\\|[^|]*\\|[^|]*\\|[^|]*\\|[^|]*\\|[^,]+)\\s*[^,]+(?=,\\s*mRNA)") ) %>%
# Verify the resulting data frame
head(ttn)

#ALTERNATE ALTERNATE ALTERNATE ATTEMPT
...
transcripts_all$target_id <- gsub(">", "", transcripts_all$target_id)
...
# Process the data, split the string by "|", and extract relevant parts
ttn <- ttn %>%
  mutate(
    # Split by the "|" character into parts
    parts = str_split(X1, "\\|"),
    
    # Check how the string splits by printing out the results for debugging
    check_split = sapply(parts, function(x) paste(x, collapse = "|")),
    
    # Extract target_id (first part before the first "|")
    target_id = sapply(parts, function(x) x[1]),
    
    # Extract gene (second part after the first "|")
    gene = sapply(parts, function(x) x[2]),
    
    # Extract product (5th part after the 4th "|", clean the mRNA part if exists)
    product = sapply(parts, function(x) {
      if (length(x) >= 5) {
        # After the 4th "|", the product is in the 5th part
        prod <- x[5]
        # Remove ", mRNA" from the end and trim any extra spaces
        prod <- str_trim(str_remove(prod, ",\\s*mRNA$"))
        return(prod)
      } else {
        return(NA)  # Return NA if there are not enough parts
      }
    })
  ) %>%
  # Select only the relevant columns for the output
  select(target_id, gene, product)

# View the result to verify the columns are extracted correctly
head(ttn)

# If necessary, you can inspect the split parts for debugging:
head(ttn$check_split)
...
#create object so
so <- sleuth_prep(metadata, full_model = ~treat, target_mapping = ttn2, extra_bootstrap_summary = TRUE, read_bootstrap_tpm = TRUE, aggregation_column = "gene")
...
transcrippts_10 <- transcripts_all %>% head(10)
...
k_DEG <- k_table %>%
  right_join(transcrippts_10, "target_id")
